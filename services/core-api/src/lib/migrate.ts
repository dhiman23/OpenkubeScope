...
//
// Keep this separate from bootstrap-admin seeding (repositories/users.ts):
// this module only guarantees the schema exists; it seeds no data.
...
// Distinct from rbac-scanner-service's and report-service's lock keys so the
// three services don't needlessly block each other's startup on the shared
// Postgres instance.
const ADVISORY_LOCK_KEY = 72001
...
export async function runMigrations(pool: Pool): Promise<void> {
  ...
  try {
    // Blocks until any other replica currently migrating releases the lock.
